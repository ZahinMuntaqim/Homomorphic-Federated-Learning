...
# ---------------------------
DATA_DIR = 'kaggleinputtomato-lef-disease-augmented'  # change to your dataset path
BATCH_SIZE = 16
...
FED_ROUNDS = 5
SEED = 42
SCALE = 106     # scale factor when encoding floats to ints
...
def image_generator(file_paths, labels, batch_size, shuffle=True)
    ...
    while True
        ...
        for start in range(0, n, batch_size)
            batch_idx = indices[startstart + batch_size]
            images = []
            batch_labels = []
            for i in batch_idx
                try
                    ...
                    print(fWarning couldn't load {file_paths[i]} {str(e)})
            if len(images) == 0
                continue
...
# ---------------------------
# Utilities for flattenunflatten weights
# ---------------------------
...
class FederatedPaillier
    ...
    def client_update_encrypt(self, client_data)
        ...
        # For the server to be able to unflatten after decrypting, we need shapessizes metadata
        # Return encrypted delta and the shapessizes metadata
    ...
    def train(self, rounds=FED_ROUNDS, frac_clients=FRAC_CLIENTS)
        for rnd in range(1, rounds + 1)
            print(fn=== Federated Round {rnd}{rounds} ===)
...
if __name__ == __main__
    ...
    print(fTotal images {len(image_paths)}, classes {class_names})
    ...
    for idx, (cx, cy) in enumerate(clients_data)
        cnt = Counter(cy)
        print(fClient {idx} samples={len(cx)}, class_dist={dict(cnt)})
    ...
    print(nEvaluating global model on test set...)
