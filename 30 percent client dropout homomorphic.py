...
class FederatedPaillier:
    ...
    def train(self, rounds=FED_ROUNDS, frac_clients=FRAC_CLIENTS, dropout_rate=0.3):
        for rnd in range(1, rounds + 1):
            ...
            # Simulate dropout: some selected clients won't return updates
            num_drop = int(dropout_rate * num_selected)
            dropped = set(np.random.choice(selected, size=num_drop, replace=False)) if num_drop > 0 else set()
            active_clients = [cid for cid in selected if cid not in dropped]

            print(f"Selected clients: {selected.tolist()}")
            print(f"Dropped (did not respond): {list(dropped)}")
            print(f"Active this round: {active_clients}")
            ...
            if len(enc_deltas) == 0:
                print("!! All clients dropped this round, skipping update.")
                continue
